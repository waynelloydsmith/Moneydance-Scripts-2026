...
# coding: utf8
# version 2/17/2025
# pick the CIBC account you want to update
# started with execfile("CIBCpicker.py") on the Jconossle .. maybe runScripts latter
# needs to fill scpirtsIPC.AccountName like runscripts does. .. ScotiaPicker isn't doing this yet
...
class CIBCpicker(JPanel, ListSelectionListener):
    import sys
...
#        print "__init__"
        ...
        global pickButton
        global accountNameDisplay
        super(CIBCpicker, self).__init__(BorderLayout())
#        UIManager.getLookAndFeel().provideErrorFeedback(None) # should beep .. not wise with moneydance
        listModel = DefaultListModel()
        i = 0
        for i in xrange(0,len(AccountNames.CIBC.Accounts)): # list of all Known CIBC Accounts
           listModel.addElement(AccountNames.CIBC.Accounts[i])
...
#            print "picked Account" ,ScotiaAccounts[index]
            accountNameDisplay.setText(AccountNames.CIBC.Accounts[index])
...
#            ret = os.system(command) # started /bin/sh .. this works but not under a moneydance console
            import scriptsIPC
            scriptsIPC.accountName = AccountNames.CIBC.Accounts[index]
            import CIBC_Inv
            CIBC_Inv.main()
#            sys.argv = ['CIBCpicker.py',AccountNames.CIBC.Accounts[index]]
#            execfile ("CIBC_Inv.py")
...
#        print "createAndShowGUI"
        ...
        frame5 = JFrame("CIBCpicker")
        frame5.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE)

        # Create and set up the content pane.
        newContentPane = CIBCpicker()
